...
def normalise_tensor(activation_tensor, dims=(2,3,4,5)): # (3,4,5)
...
def read_files(file_list, layer="blocks.39"):
    tensor_list = []
    for file_path in file_list:
        with open("experiments/activations/"+file_path+".pickle", 'rb') as handle:
            tensor_dict = pickle.load(handle)
            tensor = tensor_dict[layer]
            normalised_tensor = normalise_tensor(tensor)
            tensor_list.append(normalised_tensor)
    return tensor_list
...
feature_tensors = read_files(vortex_files_full_set)
control_tensors = read_files(laminar_files_full_set)
...
save_tensor(injection_tensor,"newTensor:(18vortex_group)-(10laminar_group)[+].pickle")
